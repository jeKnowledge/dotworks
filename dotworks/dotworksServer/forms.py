...
class StudentRegisterForm(forms.Form):
    ...
    password = forms.CharField(
        widget=forms.PasswordInput(), label="Introduza a sua password", 
        required = True)
    description = forms.CharField(
        widget= forms.Textarea, max_length = 500, 
        label="Um texto sobre si [max:500car]", required = True)
    ...
    facebook = forms.URLField(
        max_length=100, label="Facebook", required = False)
    ...
    birth_date = forms.DateField(
        widget=forms.DateInput(),label="Data nascimento", 
